...
public class CompareHistoryItemAdapter extends PageRecyclerViewAdapter<CompareHistoryItemAdapter.Holder, SearchCompareItem>{

    private CompositeDisposable compositeDisposable;

    private String targetPhotoUrl;

    public CompareHistoryItemAdapter(List<SearchCompareItem> dataLast) {
        super(dataLast);
    ...
    public void onBindHolder(Holder holder, int index, int position) {
        SearchCompareItem item = mDataLast.get(index);
        if(item.isInitInfo()) {
    ...
    static class Holder extends RecyclerView.ViewHolder{
        ...
        private void setPersonInfo(PersonInfo personInfo){
            if(personInfo == null){
                name.setText("姓名:");
                idCard.setText("身份证号:");
                nationality.setText("民族:");
                libName.setText("目标库:");
            }else {
                name.setText(TextUtil.format("姓名:%s", personInfo.getName()));
                idCard.setText(TextUtil.format("身份证号:%s", personInfo.getCode()));
                nationality.setText(TextUtil.format("民族:", personInfo.getRace()));
                libName.setText(TextUtil.format("目标库:%s", LibManager.getLibName(personInfo.getFaceSetId())));
            }
        }
    }
}
